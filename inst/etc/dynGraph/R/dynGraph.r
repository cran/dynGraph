			...
			for(numNom in 1:nbNom){
				...
				for(i in 1:dim(quali)[1]){
					if(!contient(mod,as.integer(quali[i,numNom]))){
						...
						.jcall(obj,,"addVal",as.character(quali[i,numNom]),as.matrix(tab))
					}
		    	...
		    	XXdes<-catdes(tablo,i,proba=1)
		     	...
		     	for (j in 1:nmod){
		        	if(!is.null(XXdes$test.chi)) {
		       			...
		       			if(!is.null(Xdes)){
			       			for (k in 1:nrow(Xdes))  .jcall(obj,,
								"add_description_mod_par_mod", names(XXdes$category[j]),
			...
			Xdesc=condes(tablo,1,proba=1)		
		 	if(!is.null(Xdesc$quanti)){
		    	for (j in 1:NROW(Xdesc$quanti)){
		           .jcall(obj,, "add_description_dim_par_cont",
						i, rownames(Xdesc$quanti)[j], Xdesc$quanti[j,2]*sign(Xdesc$quanti[j,1]))
		       }
		    }
		   	if(!is.null(Xdesc$quali)){		
				...
				for (j in 1:NROW(Xdesc$category)){
		           	.jcall(obj,, "add_description_dim_par_mod",
						i, rownames(Xdesc$category)[j], Xdesc$category[j,2])
		       }
	   			...
	   			if(!is.null(Xdesc$quali)){	
	     			for (j in 1:NROW(Xdesc$quali)){
	           			.jcall(obj,, "add_description_cont_par_nom",
							colnames(tablo)[k], rownames(Xdesc$quali)[j], Xdesc$quali[j,1])
	       			}
	
					for (j in 1:NROW(Xdesc$category)){
					if(Xdesc$category[j,1]>0){
	           			.jcall(obj,, "add_description_cont_par_mod",
#							colnames(tablo)[k], rownames(Xdesc$category)[j], Xdesc$category[j,2])
							colnames(tablo)[k], rownames(Xdesc$category)[j], 1)
							}
					if(Xdesc$category[j,1]<0){
	           			.jcall(obj,, "add_description_cont_par_mod",
#							colnames(tablo)[k], rownames(Xdesc$category)[j], Xdesc$category[j,2])
